...
const MENU_ITEMS = [
    ...
    { id: 'team_goals', label: 'DX팀 목표', icon: '<path d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714a2.25 2.25 0 00.659 1.591L19 14.5M14.25 3.104c.251.023.501.05.75.082M19 14.5l-1.5 4.5h-11L5 14.5m14 0H5" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>', roles: ['admin', 'user'], path: '/team-goals' },

    { id: 'weekly_report', label: '월간업무공유', icon: '<path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>', roles: ['user', 'admin'], path: '/monthly-report' },
...
window.openDivisionGoalModal = function() {
    ...
    `;
    openModal('2026년 운영본부 목표', content, null, true);
};
...
function renderCurrentView() {
    ...
    else if (STATE.currentView === "team_goals") renderTeamGoalsDX(content);
...
function generateMonthlyPeriods() {
    ...
    const endYear = now.getFullYear(), endMonth = now.getMonth() + 4; // 현재+3개월
    let y = startYear, m = startMonth;
    while (y < endYear || (y === endYear && m <= endMonth)) {
        const lastDay = new Date(y, m, 0).getDate();
        periods.push({
            key: `${y}-${m}`,
            year: String(y), month: String(m), week: '1',
            label: `${y}년 ${m}월`,
...
function getTodayMonthKey(periods) {
    const now = new Date();
...
function renderWeeklyReport(container) {
    ...
    if (!STATE.weeklyReportSelectedWeek || !periods.find(p => p.key === STATE.weeklyReportSelectedWeek)) {
        STATE.weeklyReportSelectedWeek = getTodayMonthKey(periods);
    }
    const selectedPeriod = periods.find(p => p.key === STATE.weeklyReportSelectedWeek) || periods[0];
    const years = [...new Set(periods.map(p => p.monthLabel))];
    ...
    periods.forEach(p => {
        h += `<option value="${p.key}" ${p.key===STATE.weeklyReportSelectedWeek ? 'selected' : ''}>${p.label} (${p.dateRange})</option>`;
    });
    h += '</select>';
    if (selectedPeriod) {
        h += `<span class="text-[13px] text-primary font-black">${selectedPeriod.label} 업무공유</span>`;
    }
    h += '</div>';
...
function renderWeeklyReportMyView(selectedPeriod) {
    ...
    // 마감 판단: 차월 3일 14:00 이후면 마감
    let isClosed = false;
    if (selectedPeriod) {
        isClosed = new Date() >= selectedPeriod.deadlineDate;
    }
    ...
    h += `<textarea id="weekly-report-content" rows="14" ${textareaDisabled ? 'disabled' : ''} placeholder="이번 달 업무 내용을 자유롭게 작성해 주세요.&#10;&#10;예) 진행한 업무, 완료된 작업, 이슈 및 해결 방법, 다음 달 계획 등" class="w-full ${textareaDisabled ? 'bg-surface-container cursor-not-allowed opacity-70' : 'bg-white'} border border-blue-100 rounded-xl px-4 py-3 text-[13px] text-on-surface outline-none focus:border-primary resize-none leading-relaxed custom-scroll">${content}</textarea>`;
    h += '</div>';
    if (!textareaDisabled) {
...
function renderWeeklyReportAllView(selectedPeriod) {
    ...
    targetMembers.forEach((m, idx) => {
        ...
        h += '<div class="flex items-center gap-3">';
        if (memberStatus && report && report.updated_at) {
...
async function renderTeamGoalsDX(container) {
    ...
    let totalWeeks = 0;
    months.forEach(m => totalWeeks += m.weeks); // 22
    ...
    // 연도 헤더 (별도 행)
    let yearHeaders = `<th class="border-r border-gray-100 bg-white" style="min-width:240px"></th><th class="border-r border-gray-100 bg-white" style="min-width:80px"></th><th colspan="${totalWeeks}" class="py-3 text-[13px] font-black text-gray-800 text-center bg-white">2026</th><th class="sticky right-0 bg-white"></th>`;
        ...
        <div class="max-w-full mx-auto">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-[18px] font-bold text-on-surface">DX팀 메인 마일스톤 간트차트</h2>
